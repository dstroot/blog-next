...
export const generateRSSFeed = (posts) => {
  ...
  const afterContent = (title, link, baseURL) => {
    return `<br><br>This post <a href=${link}>${title}</a> appeared on <a href=${baseURL}>Dan Stroot's Blog</a>`;
  };
  ...
  filteredPosts.forEach((post) => {
    ...
    feed.addItem({
      ...
      description: `<img src="${BASE_URL}${post.coverImage}" alt="post image" /><br><br>${post.excerpt}${after}`,
      ...
      // image: s`${BASE_URL}${post.coverImage}`,
    });
  });
