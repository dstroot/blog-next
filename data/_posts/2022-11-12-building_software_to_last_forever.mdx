...
When companies decide to re-write or replace an existing software application because it is no longer meeting their needs, they are making a similar decision. Existing software is abandoned (along with its cumulative investment) for "something new and better". Yet the belief that new code is better than old is patently absurd. Old code has been used. It has been battle-tested. Bugs have been found (lots), and more importantly fixed. It has weathered and withstood the test of time. New and better code also means new and better bugs. 
...
According to the [Guiness Book of World Records](https://www.guinnessworldrecords.com/world-records/636196-oldest-software-system-in-continuous-use) the oldest software system in use is likely either the SABRE Airline Reservation System (introduced in 1960), or the IRS Individual Master File (IMF) and Business Master File (BMF) introduced in 1962–63. The uncertainty lies in proving continuity from the earliest versions of these programs to their present-day equivalents. 

The IRS systems are the more probable candidates. As much as 20 million lines of the IMF's code is reportedly written in Assembler – a major obstacle to any modernization, as this code cannot be run on anything other than an IBM mainframe.

On the other hand, in 2020 Sabre signed a [ten year deal with Google](https://www.sabre.com/insights/releases/sabre-forges-10-year-partnership-with-google-to-build-the-future-of-travel/) to migrate SABRE to Google's cloud infratstructure. So it appears the title will likely remain with the IRS systems.

What is the secret to the long lifespan of these systems? Shouldn't companies with very long lifespan products (annuities, life insurance, etc.) study these cases? After all, they need systems that last most of a human lifespan. 
...
Airlines recognize that the value of maintenance. Much like the New York MTA's scheduled maintenance system, Airlines created a system called "progressive maintenance" that works, very, very well. 
