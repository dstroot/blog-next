...
- Why can't software last forever?
- Maintenance vs Repair/Proper maintenance needed to make software last
...
When companies decide to re-write or replace an existing software application because it is no longer meeting their needs, they are making a similar decision. Existing software must be abandoned (along with its cumulative investment) for "something new and better". Yet the belief that new code is better than old is patently absurd. Old code has been used. It has been battle-tested. Bugs have been found (lots), and more importantly fixed. It has weathered and withstood the test of time.
...
>"Isn't "preventative maintenance" a redundant term?" 

Airlines recognize that the value of maintenance. Much like the New York MTA's scheduled maintenance system, Airlines have spent years (and millions of dollars) creating a system called "progressive maintenance" that works, very, very well. 
...
In the case of the Inca rope bridges what was the cost of maintenance annually? Let's assume some of the build work was site preparation and building the stone anchors on each side, but the majority of the work was constructing the bridge itself. Since the bridge was _entirely replaced_ each year, the maintenance costs could be as much as 80% of the initial build effort, year after year. 

In the case of a brand new software system, that is well architected, well designed and built, and meets all reliability, scalability and performance needs (basically no sofware ever...) it's conceivable that there is no maintenance necessary for some period of time. Maintenance budget = 0. 

This is similar to buying a brand-new car.  The maintenance costs are negligeable in the first couple years, until they start to creep up. As the car ages the miantenance costs continue to increase until at some point it makes economic sense to buy another new car. _Except none of us wait that long._ Moat of us buy new cars before our old one is completely worn out. Yet in Cuba cars have been maintained meticulously for 30-40 years and some even run better than new.

The challenges are budgeting the appropriate amount, and p

#### Creating a maintenance fund

The challenge is that the costs of proper maintenance are choppy, and in most cases unpredicatble. In addition, there is some amount of discretion that can be applied. When your house needs a new roof it's reasonable to defer it through summer, but it probably needs to be done before winter.  

Since business require predictablity of costs, unpredictable maintenance costs are easy to defer. "we didn't budget for that, we'll have to put it in next year's budget." Except of course in the busget process it will compete with other projects and enhancement work were it's highly likely to be deprioritized. What's the solution?
...
Even so, eventually the "replacement" project will be funded (at a much higher expenditure than maintaining and rehabilitating the existing system). Why not authorize that multimillion-dollar rewrite if the engineers  convince management the existing system is doomed? Even if the executives are not listening to the engineers they will be listening to external consultants telling them they have a "burning platform" and they are falling behind. 
...
Future-proofing means constantly rethinking and iterating on the existing system. We know from both research and experience that iterating and maintaining existing solutions is much more likely, and less expensive way to improve software's lifespan and functionality. 
...
Bad software is unmaintained software. Due to factors discussed above, software does not always receive the proper amount of maintance to remain healthy. Eventually a larger modernization effort (or replacement) becomes necessary to restore a system to operational and functional excellence.
...
- 
 ...
 ([Location 3894](https://readwise.io/to_kindle?action=open&asin=B08CTFY4JP&location=3894))
