...
Proper maintenance needed to make software last
...
When companies decide to re-write or replace an existing software application because it is no longer meeting their needs, they are making a similar decision. Existing software must be abandoned (along with its cumulative investment) for "something new and better". Yet the belief that new code is better than old is patently absurd. Old code has been used. It has been battle-tested. Lots of bugs have been found, and more importantly fixed. It has weathered and withstood the test of time.
...
There is a reason that aircraft don’t regularly fail, despite there being millions of flights per year on aircraft many years old with thousands of moving parts: _maintenance_. Airlines recognize that the value of addressing problems _before they happen_. Much like the New York MTA's scheduled maintenance system, Airlines have spent years (and millions of dollars) creating a system called "progressive maintenance" that works, very, very well. 
...
Once a system degrades it is an even bigger challenge to invest in deferred maintenance (or "technical debt"). No one wants to pay for it and initiatives to repair and restore operational excellence gradually, much the way one would fix up an old house, tend to have few volunteers among engineering teams. No one gets noticed doing maintenance. No one gets promoted because of maintenance. Instead, these are the jobs that are either eliminated or offshored to less skilled, less knowledgeable, and of course cheaper labor. 

Recent price increases for construction materials like lumber, drywall, and wiring (and frankly everything else) should, according to Economics 101, force us to treat our homes more dearly. Similarly, price increases for software engineers should force companies to treat software more dearly. This kind of pragmatism is sorely needed by companies that are trying to be frugal.
...
Due to factors discussed above, software does not always receive the proper amount of maintance to remain healthy. Bad software is unmaintained software. Eventually a larger modernization effort becomes necessary to restore a system to operational and functional excellence.
