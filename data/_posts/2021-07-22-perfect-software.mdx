...
Bridges (all structures really) are intentionally built much stronger than needed for normal usage to allow for small miscalculations, emergency situations, unexpected loads, misuse, and degredation.
...
As discussed, engineers design bridges with built-in safety margins to guard against unforeseen circumstances (unexpectedly high winds, corrosion causing joints to weaken, a traffic accident severing support cables, et cetera); secure and reliable software could (and should) likewise be designed to tolerate failures within individual components.

The concept of "security in depth" is not new to network administrators; software engineers must apply the same engineering principles within individual applications as well. In the real world where components fail, it can mean the difference between being compromised or not.
