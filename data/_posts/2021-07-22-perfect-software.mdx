...
### Managing risk, engineering safe and secure software

Attacking a bridge, anonymously, from halfway around the world is hard. Attacking a website remotely is trivial by comparison.

Virtual space allows attackers to strike from anywhere and maintain anonymity. Attackers can replicate an attack easily without much additional effort/cost on their part. Attackers can even purchase "blueprints" for an attack that are basically the same thing as the attack itself. The financial motive is particularly important because it funds the ever growing arms race between offense & defense. In the physical space this kind of arms race is only visible in nation states whereas in the virtual space both nation states & private actors participate.

Assessing the security of software via the question "is it secure?" is like assessing the structure of a bridge by asking the question "has it collapsed (or been blown up) yet?" -- it is an important question, to be certain, but it also profoundly misses the point.

Engineers design bridges with built-in safety margins to guard against unforeseen circumstances (unexpectedly high winds, corrosion causing joints to weaken, a traffic accident severing support cables, et cetera); secure and reliable software could (and should) likewise be designed to tolerate failures within individual components.

The concept of "security in depth" is not new to network administrators; but software engineers must apply the same engineering principles within individual applications as well. In the real world where components fail, it can mean the difference between being compromised or not.
