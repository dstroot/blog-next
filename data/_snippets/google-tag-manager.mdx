...
In this blog post, we'll go through how to set up Next.JS with Google Analytics using the latest versions (Next.JS 12, Google Tag Manager, and Google Analytics 4).
...
This is surprisingly the simplest step. All you have to do is inject the GTM script in your `pages\_document.js` file. This is the best place to do it and it ensures GTM will be loaded at the top of your `head`.

NOTE: A [Nextjs Custom Document](https://nextjs.org/docs/advanced-features/custom-document) is commonly used to augment your application's `html`, `head` and `body` tags.

<CodeBlock>

```js:_document.js {4-18, 24}
...
![GTM Setup](/assets/snippets/img/gtm-setup.png)
