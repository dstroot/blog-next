...
I have been experimenting with Next.js 13, and the new `/app` folder for routing.  In order for a good user experience I wanted to highlight the active link in the navigation menu. Like so:

![Active Menu Item](/assets/snippets/img/menu.jpg)

In order to make this work I extended the Next.js Link component to add specific CSS classNames to a menu item when the href attribute matches the current URL. 

<Step number={1} title='Create a Custom NavLink Component' />

This is the NavLink component, by default the "active" classNames are added when the href matches the start of the URL pathname, use the `exact` property to change it to an exact match with the whole URL pathname.
...
Here is a quick example of how to use the custom NavLink component in a Next.js app.
