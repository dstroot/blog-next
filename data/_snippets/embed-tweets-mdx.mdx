...
Maxime Heckel figured out a way and documented it on [his blog](https://blog.maximeheckel.com/posts/static-tweets-with-mdx-nextjs/), but it seemed sketchy. 

### My Plan

1. Since MDX files can use/import React components I will create the following component: <CustomTweet id="abcdef123"/> where the id prop contains the id of the tweet I want to render. I want to be able to simply drop a Tweet into any of my .mdx files at any time by simply coding <CustomTweet id="abcdef123"/> and "Boom!", done. I want this component to render a beautiful Tweet, with dynamic light/dark modes. To make this work we need a `<CustomTweet>` component that will be rendered in the browser.  This component will take the Tweet id, go get the tweet, and then render it.
3. I will also create a custom API to fetch the tweet. For the component above to retrieve a tweet we can re-use Lee's code and turn it into a Next.js API. This will return the Tweet data in JSON format.
4. Finally we will use a tailwind styled component to render the tweet. In this case it's our `<Tweet>` component. This is also based off Lee's code.

Let me show you the results first. Here is my version:

<CustomTweet id='531834493922189313' />

And Twitters native version:
